  ...
  String page = R"rawliteral(
    ...
    <head>
      ...
      <style>
        ...
        .delete-container, .time-container { margin-top: 20px; text-align: center; }
        .delete-container button, .time-container button { padding: 10px 20px; background-color: #FF4136; color: white; border: none; border-radius: 5px; cursor: pointer; }
        .delete-container button:hover, .time-container button:hover { background-color: #e3342f; }
    ...
    <body>
      ...
      <script>
        ...
        function adjustTime() {
          ...
          .then(data => {
            console.log('Success:', data);
            alert('Time adjusted successfully!');
          })
          .catch((error) => {
            console.error('Error:', error);
...
void handleAdjustDateTime() {
  if (server.method() == HTTP_POST) {
    ...
    if (error) {
      server.send(400, "application/json", "{\"status\":\"failed\",\"message\":\"Invalid JSON\"}");
      return;
    }
    ...
    if (strptime(datetime, "%Y-%m-%dT%H:%M:%S", &tm) == NULL) {
      server.send(400, "application/json", "{\"status\":\"failed\",\"message\":\"Invalid DateTime format\"}");
      return;
    }
    ...
    int day = tm.tm_mday;
    int month = tm.tm_mon + 1; // tm_mon is months since January (0-11), so add 1 to get 1-12
    int year = tm.tm_year + 1900; // tm_year is years since 1900, so add 1900 to get the actual year

    rtc.adjust(DateTime(year , month , day, hours , minutes ,10)); // adjust rtc time when clicked.

  } else {
    server.send(405, "application/json", "{\"status\":\"failed\",\"message\":\"Method Not Allowed\"}");
  }
}
...
void setupOTA() {
  ...
  server.on("/adjust-time",HTTP_POST,handleAdjustDateTime);
