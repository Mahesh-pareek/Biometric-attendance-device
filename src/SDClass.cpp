...
time_t TotalAttendance::addxHours(const char * dateIN, const char* timeIN, int x) {
  char cyearIN[5];
  char cmonthIN[3];
  char cdayIN[3];
  ...
  time_t futureTime = startTime + x*3600;
...
bool TotalAttendance::newMarked(fs::FS &fs, const char *dateMark, const char *timeMark) {
  ...
  if (currentIO[0] == '1') {
    ...
    if (timediff>12*3600) {
      ...
      time_t futureTime = addxHours(dateIN, timeIN, 10);
