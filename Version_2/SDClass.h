...
class TotalAttendance : public genfile {
  public:
    ...
    bool newMarked(fs::FS &fs, const char* dateMark, const char* timeMark);

    time_t addxHours(const char * dateIN, const char * timeIN, int x);

    void appendAttendance(fs::FS &fs, const char* dateIN, const char* timeIN, const char* dateOUT, const char* timeOUT, uint32_t timediff);
...
class AttendanceSystem : public genfile{
  public:
    ...
    String returnCurrentIn(fs::FS &fs);
    String returnState(fs::FS &fs, String empID);
 };
