...
void handleSaveConfig() {
  if (server.method() == HTTP_POST) {
    ...
    Serial.println("Google Sheets URL: " + googleUrl);
...
//----------------------------------------------------------------------
void handleRoot() {
  if (!server.authenticate(wwwid_, wwwpass_)) {  // if authentication fails
...
void handleEnrollUser() {
  ...
  if (server.method() == HTTP_POST) {
    ...
    // Ensure the name is exactly 20 characters long
    if (name.length() < 20) {
      name += String(' ', 20 - name.length());
    }
